...
const Codes = () => {
  ...
  return (
    <div className="flex flex-col gap-8 px-4 sm:px-6 md:px-8 py-8 bg-gray-100 min-h-screen">
  <div className="flex flex-col lg:flex-row justify-center items-start gap-8 pt-16 w-full">
    <div className="flex-1 max-w-lg w-full">
      <div className="bg-white px-6 sm:p-8 rounded-xl shadow-xl w-full min-h-[460px] flex flex-col justify-center">
        <h2 className="text-xl sm:text-2xl font-semibold text-center mb-6 text-gray-700">Generate Code</h2>
        <button
        ...
        {code && (
          <div className="text-center mt-6 p-4 bg-gray-50 border rounded-lg">
            ...
            <p className="text-xl sm:text-2xl font-bold text-blue-500 tracking-wider">{code}</p>
            <button
    ...
    <div className="flex-1 max-w-lg w-full bg-white shadow-lg rounded-xl p-6">
      <h1 className="text-xl sm:text-2xl font-semibold text-gray-700 mb-6 text-center">Codes</h1>
      {loading ? (
        ...
        <ul className="space-y-3">
          {codes.map((code, index) => (
            <li
              key={index}
              className="flex flex-col sm:flex-row justify-between items-center p-4 bg-gray-50 border rounded-lg"
            >
              <span className="text-gray-800 font-medium break-all">{code}</span>
              <button
                onClick={() => handleSelect(code)}
                className="mt-2 sm:mt-0 bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition"
              >
                Select
              </button>
            </li>
